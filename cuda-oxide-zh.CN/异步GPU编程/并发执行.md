...
如果 GPU 有足够的 SM 同时运行多个内核，重叠版本会明显更快。并且由于每个流水线都是同一个流上的单个 `and_then` 链，批次内的各个阶段仍然是严格有序的——不需要跨流同步。
...
每个 `and_then` 闭包通过 `move` 捕获数据。内核启动产生 `()`，因此你需要显式地将下一阶段需要的缓冲区向前传递：

```rust
launch_gemm(input, hidden, w0)
    .and_then(move |()| {
        // input 被内核消耗。hidden 和 module 幸存，
        // 因为它们被捕获但没有被消耗。
...
| 工作负载 | 建议池大小 | 原因 |
|----------|------------|------|
| 每次启动一个大内核 | 1–2 | 内核已经使 GPU 饱和 |
| 许多小内核 | 4–8 | 在流之间重叠启动开销 |
| 内核 + memcpy 混合 | 2–4 | 将计算与数据传输重叠 |
...
寻找同一流上内核之间的间隙（启动开销）、池中空闲的流（工作分配不均衡）以及跨流之间意外的串行化（缺少或多余的依赖关系）。
...
| [上一页](./调度和流.md) | [下一页](../构建一个真正的应用程序/项目-异步MLP管道.md) |
| :--- | ---: |