...
| libNVVM      | `libnvvm.so`（CUDA Toolkit）可加载 —— libdevice 数学kernel所需           |
| nvJitLink    | `libnvJitLink.so`（CUDA Toolkit）可加载 —— libdevice 数学kernel所需      |
| libdevice    | `libdevice.10.bc` 可发现 —— libdevice 数学kernel所需                   |
...
| kernel中使用 `panic!("message")` | 编译错误（格式化不可用） | 使用 `gpu_assert!` 或 `debug::trap()` |
...
| [上一页](./闭包和泛型.md) | [下一页](../03-GPU安全/安全模型.md) |
| :--- | ---: |