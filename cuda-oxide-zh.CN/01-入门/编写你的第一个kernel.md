# 编写你的第一个kernel — cuda-oxide

本节将介绍安装 cuda-oxide、创建项目、编写 GPU kernel并运行它 —— 全部使用纯 Rust 完成。
...
这会检查兼容的 GPU、CUDA toolkit、LLVM 和代码生成后端。在继续之前，请先修复它报告的所有问题（详见[安装指南](./安装.md)）。
...
└── src/
    └── main.rs          # kernel代码 + 主机代码在同一文件中
```

构建并运行它：

你应该会看到 `PASSED: all 1024 elements correct`。生成的模板是一个向量加法kernel —— 不错的起点，但我们来看点更有趣的。

---

## kernel剖析

这里是一个略有不同的向量加法：元素级加法被提取到一个普通的辅助函数中。kernel和辅助函数与主机代码一起位于同一个文件中：
...
kernel和主机代码**位于同一个文件**中，通过单次 `cargo` 命令调用进行编译。代码生成后端拦截编译过程，将 `#[kernel]` 函数路由到 MIR-to-PTX 流水线，其余部分委托给标准 LLVM。最终二进制文件包含原生主机代码以及嵌入的设备产物。

### `#[kernel]`

将函数标记为**可启动的kernel入口点** —— 相当于 GPU 版的 `main`。该函数通过以下流水线编译为 PTX：
...
上面的 `add` 辅助函数**没有任何注解**。当编译器处理 `#[kernel]` 时，它会遍历调用图并自动发现kernel调用的每个函数。这些函数会被编译为 PTX 设备函数，并由后端内联 —— 你无需手动标记它们。

> **注意**
> 
> `#[device]` 属性存在，但用途不同：它将函数标记为独立的设备编译根（用于构建 C++ 消费的 Rust 设备库），或在 `#[device] extern "C" { ... }` 块中用于声明与 CUDA C++ LTOIR 进行 FFI 的外部设备函数。对于从kernel调用的私有辅助函数，你**不需要** `#[device]`。

### `#[cuda_module]`

`#[cuda_module]` 包装内联kernel模块并生成类型化的主机 API：
...
加载器从主机二进制文件中读取嵌入的设备产物，缓存kernel函数句柄，并将每个 `#[kernel]` 暴露为 Rust 方法。方法签名与kernel签名一致，设备切片映射为 `DeviceBuffer` 借用。

`load_kernel_module` 和 `cuda_launch!` 作为底层 API 仍然可用，用于手动加载附属产物和自定义启动代码。

### 参数标量化

切片通过主机/设备 ABI 时，以其 `(ptr, len)` 组件传递 —— 主机将它们作为两个kernel参数传递，设备编译器在入口基本块中重新组装切片。按值传递的结构体和闭包则作为一个 byval `.param` 传递，因此主机数据包将整个聚合体作为单个槽位推送（这与启动器实际行为一致，避免了按字段声明的不匹配）。所有这些都是完全透明的 —— kernel签名看起来仍然是普通的 Rust：
...
对于多kernel流水线或并发工作负载，cuda-oxide 提供了基于 Tokio 的异步执行模型。我们来脚手架创建一个异步项目，并讲解其中的差异。
...
kernel本身是**完全相同的** —— 异步只改变你在主机端启动和管理 GPU 工作的方式。
...
用 `.and_then(|result| next_op)` 链式连接依赖操作。用 `zip!(op_a, op_b)` 并发运行独立操作 —— 两者都提交到流池，当两者都完成时组合结果可用。这些组合子让你能够以声明式方式表达复杂的多kernel流水线。

> **提示**
> 
> 更完整的异步示例，请参阅 `async_mlp` —— 一个多kernel前向传播（GEMM、MatVec、ReLU），使用 `and_then` 链式调用、`zip!` 并行分配，以及跨并发批次的 `Arc` 共享权重。在 cuda-oxide 工作区中运行 `cargo oxide run async_mlp` 即可体验。

| [上一页](./安装.md) | [下一页](../02-编写GPU程序/cuda执行模型.md) |
| :--- | ---: |