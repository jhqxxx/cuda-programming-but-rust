...
与任何编程模型一样，CUDA也依赖底层硬件的概念模型。就CUDA编程的而言，GPU可以被看成是*Streaming Multiprocessors(SM)-流式多处理器*的集合，这些SM被组织成叫*Graphics Processing Clusters(GPCs)-图形处理簇*的组。每个SM拥有一个本地寄存器文件，一个统一数据缓存和多个执行计算的功能单元。统一数据缓存为共享内存和L1缓存提供物理资源。统一数据缓存到L1和共享内存的分配可以在运行时配置。不同类型的内存大小和SM内的功能单元数量在不同的GPU架构中可能存在不同。
...
| :--: |
| *图2*：GPU有更多的流式多处理器（SM），每个SM拥有多个功能单元。<br> 图形处理簇（GPC）是SM的集合。<br> GPU是连接到GPU内存的GPC集合。<br> CPU通常包含多个核心和一个连接到系统内存的内存控制器。<br> CPU和GPU通过像PCIe或NVLINK的之类的互联技术连接。 |
...
#### 1.2.2.1.1 Thread Block Clusters-线程块簇
除了线程块之外，计算能力为9.0及以上的GPU还具有一个叫*cluster-簇*的可选分组级别。cluster是一组线程块，它与线程块和网格类似，也可以是1维、2维或3维的。
...
指定cluster会将相邻的线程块分组到cluster中，并在cluster级别增加一些同步和通信的额外机会。具体来说，cluster内部的所有线程块都在单个GPC（图形处理簇）中执行。<u>[图6](../assets/img/thread-block-scheduling-with-clusters.png)</u>展示了当指定cluster后，GPC中如何将线程块调度到SM。由于线程块是在单个GPC上同时调度的，因此同一cluster内不同块内的线程可以使用<u>[*Cooperative Groups-合作组*]()</u>提供的软件接口互相通信和同步。cluster内的线程可以访问cluster内所有块的共享内存，这叫做*distributed shared memory-分布式共享内存*。cluster的最大规模取决于硬件，并且因设备而异。
...
除了前面章节描述的 SIMT 模型之外，CUDA 还支持 tile 编程模型。在 tile 编程中，程序员在整个线程块的级别编写代码，描述对称为 **tiles** 的多维数据集合的操作。编译器将这些操作映射到块中的各个线程。

Tile 内核在块网格上启动，如[线程块和网格](#programming-model-threads-grids)部分所述。每个块执行 tile 内核，并可以查询其在网格内的位置，以确定它负责哪部分数据。程序员只指定网格维度；每块线程数由编译器根据内核中的 tile 操作决定（[图 8](../assets/img/tile-simt.png)）。

<figure class="align-center" id="figure-tile-programming-abstraction">
<img alt="SIMT 和 tile 编程模型中程序员的视角" src="../assets/img/tile-simt.png" style="width: 454px;" />
<figcaption>
<p><span class="caption-number">图 8 </span><span class="caption-text">SIMT 和 tile 编程模型中程序员的视角。在 SIMT 中，程序员编写每线程代码并控制每个线程如何访问数据。在 tile 编程中，程序员编写操作 tiles 的每块代码；编译器将操作映射到块中的线程。</span></p>
</figcaption>
</figure>

在一个 tile 内核中，块执行单个控制流。程序员指定对 tiles 的操作，编译器将工作分配到块的线程中。支持标准的控制流结构，如条件判断和循环，但由于块遵循单个控制流，因此不存在 warp 发散的概念。标量操作，如计算索引或循环边界，由块中的单个线程执行。Tile 操作，如逐元素相加两个 tiles，由块中的所有线程共同并行执行。

重要的是不要混淆块（执行单元）和 tiles（数据单元）。一个块可以创建并操作许多不同形状和数据类型的 tiles。

#### 1.2.2.3.1. 数组和 tiles

Tile 内核处理两种类型的数据：**数组** 和 **tiles**。数组（或全局数组）是存储在设备内存中的元素的多维容器。数组是可变的：其内容可以通过内核中的存储操作进行修改。一个数组具有形状和数据类型。

一个 tile 是值的多维集合，仅存在于 tile 代码中，并且是单个块的本地数据。Tiles 是不可变的：对 tile 的每个操作都会产生一个新的 tile，而不是修改现有的 tile。与数组不同，tile 不一定在内存中有表示形式——编译器决定 tile 数据的存储方式，并且可以使用寄存器、共享内存或 SM 的其他资源。Tile 的每个维度必须是 2 的幂，并且必须在编译时已知（即其值必须在内核执行前可确定，而不是在执行期间计算）。Tiles 不能作为内核参数传递；它们完全在 tile 代码内部创建和使用。

#### 1.2.2.3.2. Tile 空间和数据移动

数据通过加载和存储操作在数组和 tiles 之间移动。这些操作使用一个称为 **tile 空间** 的概念，该概念是将一个数组概念性地划分为大小相等、不重叠的 tiles 的结果。例如，考虑一个形状为 (M, N) 的二维数组。如果一个加载操作指定了一个形状为 (t<sub>m</sub>, t<sub>n</sub>) 的 tile，那么该数组被概念性地划分为 ⌈M/t<sub>m</sub>⌉ 行和 ⌈N/t<sub>n</sub>⌉ 列的 tiles。指向此 tile 空间的索引，例如 (i, j)，标识要加载哪个 tile。加载返回一个形状为 (t<sub>m</sub>, t<sub>n</sub>) 的 tile，其中包含数组中相应的元素。当一个 tile 延伸到数组边界之外时——例如，当数组维度不是 tile 维度的整数倍时在边缘处——加载指定如何处理越界元素，例如用零填充它们（[图 9](../assets/img/tile-data-movement.png)）。

<figure class="align-center" id="figure-tile-space-data-movement">
<img alt="Tile 空间和数据移动" src="../assets/img/tile-data-movement.png" style="width: 454px;" />
<figcaption>
<p><span class="caption-number">图 9 </span><span class="caption-text">Tile 空间和数据移动。一个形状为 (M, N) 的二维数组被概念性地划分为一个形状为 (t<sub>m</sub>, t<sub>n</sub>) 的 tiles 网格。在 tile 空间索引 (i, j) 处的加载返回相应的 tile。在数组边界处，落在数组外部的元素可以用零填充。存储操作将 tile 写回数组中给定的 tile 空间索引处。</span></p>
</figcaption>
</figure>

存储操作执行相反的操作：给定一个 tile 和一个指向 tile 空间的索引，它将 tile 的元素写入数组的相应区域。任何落在数组边界之外的写入都会被静默丢弃。Tile 程序也支持聚集和散射操作，它们从数组中的任意位置加载或存储到任意位置。

#### 1.2.2.3.3. 对 tiles 的操作

Tile 程序提供了一组作用于 tiles 的内置操作，包括逐元素算术、矩阵乘法、沿一个或多个轴的规约（如求和、最大值）、形状操作（如重塑、转置）和类型转换。当两个不同形状的 tile 在操作中组合时，较小的 tile 会在应用操作之前自动扩展以匹配较大的 tile。

#### 1.2.2.3.4. 与 SIMT 编程的关系

Tile 编程和 SIMT 编程在 CUDA 中共存。一个应用程序可以同时包含 SIMT 和 tile 内核，并且两种类型的内核都可以操作设备内存中的相同数据。编程模型的选择是每个内核的决定。Tile 编程不会取代 SIMT 编程。SIMT 提供对单个线程的细粒度控制，这对于某些算法和优化技术仍然是必要的。Tile 编程提供了一个更高层次的抽象，可以简化内核开发。由于线程级决策留给编译器，相同的 tile 内核可以在不同的 GPU 架构上运行，而无需更改源代码。两种模型都构建在相同的底层硬件之上——前面章节中描述的 SM、线程块和网格。两种模型也使用相同的设备内存空间，这将在下一节中介绍。
